-- [v6 2026-07-07] Exclude auto-logged activity (game/lesson/article sessions)
-- from the feed's practice-session line so 1-minute game logs don't clutter it.
...
sessions AS (
  ...
  WHERE ps.email NOT IN (SELECT email FROM excl)
    -- Exclude auto-logged in-app activity (games / lessons / articles) from the
    -- feed's "logged a practice" line so a 1-min game play doesn't read as a
    -- practice session or clutter the feed. Those already surface via their own
    -- event lines (first play, high score). Manual + quick logs still show.
    AND (ps.source IS NULL OR ps.source NOT IN ('app-game','app-lesson','app-article'))
