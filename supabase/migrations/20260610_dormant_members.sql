-- Server-side dormant-member cohort: members (allowed_emails) whose last activity
-- across the whole app is more than `days` ago (or who have no activity at all).
--
-- Mirrors exactly the sources the admin "Reactivate" computation uses, so the
-- Email Studio's Relaunch/Reactivation cohort matches the analytics page — and,
-- crucially, it runs server-side, so the future scheduled reactivation trigger
-- can compute who's gone quiet without a browser open.
--
-- SECURITY DEFINER so it can read across activity tables, but it refuses to run
