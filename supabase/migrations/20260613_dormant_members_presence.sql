-- Fix: the reactivation/dormant cohort ignored the app heartbeat (user_presence),
-- so members who signed in and just browsed (no post / practice / game / feature
-- event) were wrongly flagged dormant and emailed the win-back. The admin
-- retention console already treats user_presence.last_seen_at as "active"; this
-- realigns the server-side dormant_members() function to the same source.
--
-- Additive only: adding user_presence to the union can only REMOVE members from
-- the dormant set (anyone the heartbeat saw within `days`), never add one.
...
BEGIN
  ...
  WITH acts AS (
    ...
    FROM (
      ...
      UNION ALL SELECT email, last_seen_at::date FROM user_presence
