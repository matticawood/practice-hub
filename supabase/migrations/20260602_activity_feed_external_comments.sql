-- [v3 2026-06-02] Extended to include comments on content-feed, weekly-focus,
-- and practice-room-update posts (post_type discriminator: content_feed|weekly_focus|update).
...
-- 4b. Content-feed comments (YouTube / articles). post_type carries a source
--     discriminator and piece_title/piece_composer carry the parent post title
--     and a comment snippet, so the client can render + deep-link without a
--     second round-trip. event_type stays comment/reply (split on parent).
cf_comments AS (
  SELECT
