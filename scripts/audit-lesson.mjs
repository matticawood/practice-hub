...
const defTriggers = {
  ...
  "major-scale pattern (full)": /T T S T T T S|W W H W W W H/g,
  "key-signature definition": /key signature is a|a key signature is/gi,
...
for (const [name, re] of Object.entries(defTriggers)) {
  ...
  if (n > 2) flag("RE-TEACH", `"${name}" appears ${n}x — should be defined ONCE then recapped (check it isn't re-taught)`);
}
// repeated headings
...
const order = ["STRUCT","NOTATION","BARS","QUIZ","STYLE","DURATION","RE-TEACH","PLAY?"];
console.log("\n================= FLAGS =================");
