...
import { leapClashes, sixFours, parallelPerfects, lhParallels, beatClash, handCrossing } from './harmony-checks.mjs';   // the genuine-fault checks (not beatDissonance, which flags desirable suspensions)
...
function genContour(nbars,span,bias){            // varied wide-range contours reaching up to `span`; `bias` forces a shape (style idiom)
  const shape=bias||rnd(['arch','arch','wave','rise','fall']); const out=[];
...
const QFEEL = {
  smooth: [[1],[1],[1],[0.5,0.5]],                                                              // legato: mostly held, occasional quavers
  lilt:   [[1],[1],[0.5,0.5],[0.75,0.25]],                                                      // graceful: dotted lilt
  crisp:  [[1],[0.5,0.5],[0.75,0.25],[0.5,0.25,0.25],[0.25,0.5,0.25],[0.25,0.25,0.5],[0.25,0.25,0.25,0.25]], // brisk: dotted + semiquavers
...
function wideBar(barU,feel,spd,march,calmMel){
  ...
  const qpool = march ? [...(QFEEL[feel]||QFEEL._def), [0.75,0.25],[0.75,0.25],[1]] : (QFEEL[feel] || QFEEL._def);
...
function barRhythm(barU,wide,compound,feel,spd,march,calmMel){
  ...
  if(!wide){ const pool=RHY[barU]||RHY[2];
    ...
    return rnd(pool); }
  return wideBar(barU,feel,spd,march,calmMel);              // grade 3+: recursive macro partition -> long notes + subdivisions
}
...
function buildCandidate(grade, opts={}){
  ...
  let melReg, accReg;
  if(swap){ melReg=lhTonic; accReg=rhT; }
  else if(chance(0.62)){                                   // WIDE: 2-octave gap
    accReg = snapNear(rhTonic, 43, 38);                    // accompaniment low (D2..Bb2), tonic-aligned
    melReg = accReg + 24;                                  // melody two octaves up (comfortable treble, peaks stay <= ~C6)
  } else { melReg=rhT; accReg=lhTonic; }                   // CLOSE: the original 1-octave spread (kept for variety)
  ...
  const SADMARK=/sadly|mesto|lament|dolente|piangevole|elegy|elegia/i;
  const markPool = mode==='maj' ? (character.marks.filter(m=>!SADMARK.test(m))) : character.marks;
  const tempo = rnd(markPool.length ? markPool : character.marks);
  ...
  const spd = (m=>{ m=(m||'').toLowerCase();
    if(/adagio|lento|larg|grave|maestoso|lullaby|sadly|mesto|slowly|sostenuto|tranquillo|cantabile|espress|tenderly|gently|dolce/.test(m)) return 'slow';
    if(/moderato|andant|allegretto|grazioso|comodo|minuet|valse|waltz|con moto|flowing|steadily/.test(m)) return 'mod';
    if(/vivace|presto|allegro|brio|risoluto|giocoso|scherz|marcia|march|ritmic|lively|leggiero|dancing|playful|vivo/.test(m)) return 'fast';
    return 'mod'; })(tempo);
  ...
  const figCands = wide ? FIGBANK.filter(f=>f.t.includes(time)&&grade>=Math.min(...f.g)&&f.f.includes(character.feel)) : [];   // CUMULATIVE: a cell's grade tag is its MINIMUM; grade 4 draws 2/3/4 cells, grade 3 draws 2/3, etc.
  ...
  const texture = swap ? (useFig ? 'fig' : rnd(['block','sustained','rootfifth','broken','alberti']))
  ...
  const contour = wide ? genContour(nbars,span,contourBias) : rnd(CONTOURS[nbars]||CONTOURS[4]);
  ...
  let motifA = barRhythm(barU,wide,compound,rhythmFeel,spd,march,calmMel);
  for(let _a=0; _a<8 && motifA.length<2; _a++) motifA = barRhythm(barU,wide,compound,rhythmFeel,spd,march,calmMel);
  const motifB = barRhythm(barU,wide,compound,rhythmFeel,spd,march,calmMel);
  const varyRhythm = m => { if(m.length<2) return m.slice(); const out=m.slice();
    const i=out.findIndex(d=>d>=2); if(i>=0 && chance(0.5)){ out.splice(i,1,out[i]-1,1); } return out; };  // split one long note
  const phraseLen = Math.max(1, half);
  const rhythmByBar = [];
  for(let b=0;b<nbars;b++){ const pos=b%phraseLen;
    rhythmByBar.push( pos===2 ? motifB : (pos%2===1 ? varyRhythm(motifA) : motifA) ); }
  const buildBar = (b, endIdx) => {
    const pat=rhythmByBar[b] || barRhythm(barU,wide,compound,rhythmFeel,spd,march,calmMel);
    ...
    for(let j=0;j<pat.length;j++){
      ...
      if(Math.abs(lastMove)>=3) tgt = lastIdx - Math.sign(lastMove);        // recover the last leap: step back
        ...
        if(lineDir>0 && toC>-2) tgt = lastIdx + 2;
             ...
             tgt = lastIdx + (chance(.62)?toward : chance(.65)?toward*2 : 0); } // mostly a step toward the contour, then a 3rd, RARELY a hold
  ...
  const pahSpacing = swap ? 'close' : rnd(['close','close','open','open']);
  ...
  prog.forEach((c,b)=>{
    ...
    const add7 = (c===Tn.V) && seventhDeg!=null && b<nbars-1 && prog[b+1]===Tn.I && chance(0.4);
  ...
  if(wide && !swap && !compound && !globalThis.__NOCTR){
    ...
    for(const n of lh){ const b=Math.floor(t/barU+1e-9);
      ...
      if(!structural && Array.isArray(n.m) && n.m.length>=2 && nb>=3 && chance(0.6)){
        ...
        const mel=melByBar[b]||[]; const melDir=mel.length>=2?Math.sign(mel[mel.length-1]-mel[0]):0; const cDir=-melDir||(chance(.5)?1:-1);
...
export function generate(grade, tries, avoid){
  ...
  for(let i=0;i<tries;i++){
    ...
    const hc = leapClashes(ex).length + sixFours(ex).length + parallelPerfects(ex).length + lhParallels(ex).length + beatClash(ex).length;   // hand collisions are REPAIRED at source (octave-shift), not rejected — see collision repair in buildCandidate
    ...
    const score=v.errors.length*1000 + hc*1000 + plain*1000 + v.warnings.length;   // prefer no errors/faults, then fewest warnings
    if(score===0){
