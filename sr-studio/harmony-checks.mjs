...
// A bare 4th above the bass on a strong beat reads as a 6/4 — weak as a statement.
export function sixFours(ex){
  ...
  for(let bar=0; bar*barU < RH.at(-1).t+RH.at(-1).d; bar++){
    ...
    if(!rn||!ln||rn.rest||ln.rest) continue;
    const s=((hi(rn)-lo(ln))%12+12)%12;
    if(s===5) out.push({bar:bar+1, note:'bare 4th above the bass on the downbeat (6/4)'});
  }
  return out;
}
...
export function beatDissonance(ex){
  const tl=s=>{let t=0,o=[];for(const n of s){o.push({...n,t});t+=n.d;}return o;};
  const RH=tl(ex.rh), LH=tl(ex.lh);
  const hi=n=>Array.isArray(n.m)?Math.max(...n.m):n.m, lo=n=>Array.isArray(n.m)?Math.min(...n.m):n.m;
  const sound=(a,t)=>{let c=null;for(const n of a){if(n.t<=t+1e-9)c=n;else break;}return c;};
  ...
  for(let t=0;t<total-1e-9;t+=beatLen){
    ...
    if(!rn||!ln||rn.rest||ln.rest) continue;
    const s=((hi(rn)-lo(ln))%12+12)%12;
    if(CONSONANT.has(s)) continue;
    out.push({bar:Math.floor(t/barU)+1, beat:+((t%barU)/beatLen+1).toFixed(2), interval:NAME[s]||s});
  }
  return out;
}
...
export function beatClash(ex){
  ...
  for(let t=0;t<total-1e-9;t+=beatLen){
    const rn=sound(RH,t), ln=sound(LH,t);
    if(!rn||!ln||rn.rest||ln.rest) continue;
    const nxt=RH[RH.indexOf(rn)+1];
    for(const r of pcs(rn)){ for(const l of pcs(ln)){ const ic=((r-l)%12+12)%12, s=Math.min(ic,12-ic);
      if(s!==1) continue;
      // A semitone dissonance is FINE when it RESOLVES: the clashing right-hand voice steps DOWN to a consonance
      // (a suspension held over and dropped, or an appoggiatura leaned on and dropped). Only flag one that just
      // sits or leaps away without resolving (Matthew's Ab, which repeats then leaps down a 3rd to F).
      const resolvesDown = nxt && !nxt.rest && (Array.isArray(nxt.m)?nxt.m:[nxt.m]).some(m => m===r-1 || m===r-2);
      if(!resolvesDown){ out.push({bar:Math.floor(t/barU)+1, beat:+((t%barU)/beatLen+1).toFixed(2), note:'unresolved semitone clash between the hands'}); break; }
    } }
  }
  return out;
}
