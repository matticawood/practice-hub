...
`;
  ...
  function _renderCommentMedia(c) {
    ...
    others.forEach(m=>{
      if(m.type==="mux")          html+=`<div style="margin-top:8px;width:min(480px,70vw);aspect-ratio:16/9;border-radius:8px;overflow:hidden;background:var(--surface)"><mux-player playback-id="${_escHtml(m.playbackId)}" controls style="width:100%;height:100%;display:block"></mux-player></div>`;
  ...
  const Comments = {
    ...
    async submitComment(parentId) {
      ...
      const auth=_auth();
      const row={[_cParent()]:parentId,email:auth.email,name:auth.name,content,media:uploaded.length?uploaded:[]};
      if(poll&&!_cfg?.disablePolls)row.poll=poll;
      const {error}=await _db().from(_cTable()).insert(row);
    ...
    async submitReply(parentId, commentId, composerEl) {
      ...
      const auth=_auth();
      const {error}=await _db().from(_cTable()).insert({[_cParent()]:parentId,email:auth.email,name:auth.name,content,parent_comment_id:commentId,reply_to_name:replyToName,media:uploaded.length?uploaded:[]});
      if(ta)ta.disabled=false;
