...
(function () {
  ...
  intervalrec:["itg1","itg10","itg50","its15","its25","its35","itread","itear","itfull","itfree"],
  ...
  const _BADGE_CAT_SHAPE={sessions:'circle',time:'hex',streak:'shield',pieces:'diamond',scales:'hex',sightread:'circle',improv:'star',theorycat:'circle',eartraining:'hex',depth:'star',books:'circle',variety:'star',saves:'shield',reading:'shield',community:'circle',game:'hex',library:'diamond',noterec:'star',chordrec:'diamond',earrec:'hex',intervalrec:'diamond',live:'star',courses:'shield',};
  const _BADGE_PALETTE={sessions:['#93c5fd','#2563eb','#1d4ed8'],time:['#fde68a','#f59e0b','#b45309'],streak:['#fca5a5','#ef4444','#b91c1c'],pieces:['#c4b5fd','#7c3aed','#5b21b6'],scales:['#6ee7b7','#059669','#047857'],sightread:['#67e8f9','#0891b2','#0e7490'],improv:['#fdba74','#ea580c','#c2410c'],theorycat:['#86efac','#16a34a','#15803d'],eartraining:['#d8b4fe','#9333ea','#7e22ce'],depth:['#fca5a5','#dc2626','#991b1b'],books:['#a5b4fc','#4338ca','#3730a3'],variety:['#f9a8d4','#db2777','#be185d'],saves:['#fde68a','#ca8a04','#a16207'],reading:['#5eead4','#0d9488','#0f766e'],community:['#7dd3fc','#0284c7','#0369a1'],game:['#fdba74','#c2410c','#9a3412'],library:['#f9a8d4','#be185d','#9d174d'],noterec:['#d9f99d','#65a30d','#3f6212'],chordrec:['#f0abfc','#c026d3','#86198f'],earrec:['#a5f3fc','#06b6d4','#0e7490'],intervalrec:['#fcd34d','#f59e0b','#b45309'],live:['#fda4af','#e11d48','#9f1239'],roadmap:['#fde68a','#f0a500','#9a6a00'],courses:['#6ee7b7','#10b981','#047857'],};
  // Overlay the shared engine's badge maps + derive category order from ACHIEVEMENTS,
  // so new games/categories/icons from shared-achievements.js appear here with nothing
  // to hand-sync (this also backfills the ear-game icons the modal was missing).
  try {
    var _AE = (typeof window !== "undefined" && window.ACH_ENGINE) || null;
    if (_AE) { Object.assign(ACH_ICON_MAP, _AE.ICON); Object.assign(_BADGE_PALETTE, _AE.PALETTE); Object.assign(_BADGE_CAT_SHAPE, _AE.SHAPE); }
    if (typeof ACHIEVEMENTS !== "undefined" && Array.isArray(ACHIEVEMENTS)) {
      Object.keys(ACH_CAT_ORDER).forEach(function (k) { delete ACH_CAT_ORDER[k]; });
      ACHIEVEMENTS.forEach(function (a) { if (a && a.id && a.cat) { (ACH_CAT_ORDER[a.cat] = ACH_CAT_ORDER[a.cat] || []).push(a.id); } });
    }
  } catch (_) {}
