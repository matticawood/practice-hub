...
(function () {
  ...
  function injectStyles() {
    ...
    .lr-body{--surface:#ffffff;--surface-2:#f4f4f7;--border:#e5e5ea;--text:#15151a;--text-muted:#6e6e7a;--accent:#f5c518;
      ...
      --d-key:#b45309;--d-tip:#047857;--d-watch:#be123c;--d-note:#0369a1;--d-do:#6d28d9;--d-quiz:#7e22ce;
    ...
    .lr-callout-label{display:flex;align-items:center;gap:9px;margin:0;padding:9px 14px;
      background:var(--bar);color:#fff;font-size:.66rem;font-weight:800;text-transform:uppercase;letter-spacing:.11em}
    .lr-callout-label::before{content:"";width:16px;height:16px;flex:0 0 auto;background:currentColor;
      -webkit-mask:var(--ico) center/contain no-repeat;mask:var(--ico) center/contain no-repeat}
    .lr-callout-key{--c:var(--c-key);--bar:var(--d-key);--ico:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18h6M10 22h4M12 2a7 7 0 0 0-4 12.7V17h8v-2.3A7 7 0 0 0 12 2z"/></svg>')}
    .lr-callout-tip{--c:var(--c-tip);--bar:var(--d-tip);--ico:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>')}
    .lr-callout-watch{--c:var(--c-watch);--bar:var(--d-watch);--ico:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><path d="M12 9v4M12 17h.01M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/></svg>')}
    .lr-callout-note{--c:var(--c-note);--bar:var(--d-note);--ico:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 16v-5M12 8h.01"/></svg>')}
    ...
    .lr-play-btn{display:flex;width:100%;align-items:center;gap:12px;text-align:left;color:#fff;
      background:linear-gradient(180deg,#7c4ff0,#5f39d0);border:none;
      border-radius:13px;padding:13px 17px;font:inherit;font-weight:700;font-size:.92rem;
      cursor:pointer;box-shadow:0 6px 18px -9px rgba(95,57,208,.95)}
    .lr-play-btn:hover{background:linear-gradient(180deg,#8b5cf6,#6d4ae0)}
    ...
    .lr-kbd-play{display:flex;width:100%;align-items:center;gap:12px;text-align:left;
      background:linear-gradient(180deg,#7c4ff0,#5f39d0);border:none;border-radius:13px;padding:13px 17px;
      font:inherit;font-size:.92rem;font-weight:700;color:#fff;cursor:pointer;box-shadow:0 6px 18px -9px rgba(95,57,208,.95)}
    .lr-kbd-play:hover{background:linear-gradient(180deg,#8b5cf6,#6d4ae0)}
    ...
    .lr-task{--c:var(--c-do);--bar:var(--d-do);margin:22px 0;padding:0;border-radius:15px;overflow:hidden;
      background:color-mix(in srgb,var(--c) 6%,#fff);border:1px solid color-mix(in srgb,var(--c) 30%,#fff)}
    .lr-task .lr-text{margin:0;padding:15px 17px 16px}.lr-task .lr-text p:last-child{margin-bottom:0}
    .lr-task-label{display:flex;align-items:center;gap:9px;margin:0;padding:9px 14px;background:var(--bar);color:#fff;
    ...
    .lr-task-share{margin:0 17px 16px;background:var(--bar);color:#fff;border:none;border-radius:10px;padding:9px 15px;font-weight:700;font-size:.82rem;cursor:pointer}
    ...
    .lr-opt.lr-sel::before{border-color:var(--d-quiz);background:var(--d-quiz);color:#fff}
    .lr-opt.lr-correct{border-color:var(--c-tip);background:color-mix(in srgb,var(--c-tip) 10%,#fff)}
    .lr-opt.lr-correct::before{border-color:var(--d-tip);background:var(--d-tip);color:#fff}
    .lr-opt.lr-wrong{border-color:var(--c-watch);background:color-mix(in srgb,var(--c-watch) 10%,#fff)}
    .lr-opt.lr-wrong::before{border-color:var(--d-watch);background:var(--d-watch);color:#fff}
    ...
    .lr-check,.lr-done,.lr-quiz-start,.lr-quiz-submit{background:var(--d-do);color:#fff;border:none;border-radius:11px;padding:10px 17px;font-weight:700;font-size:.85rem;cursor:pointer}
